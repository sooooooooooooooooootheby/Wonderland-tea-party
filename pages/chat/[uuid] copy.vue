    ...
    <div class="room">
        <v-virtual-scroll :items="chat.chat" class="messageBox" v-if="chat.chat && chat.chat.length > 0">
            <template v-slot:default="{ item, index }">
                <div class="box" :ref="index === chat.chat.length - 2 ? 'secondLast' : null">
        ...
        <button class="btn btn-circle btn-outline btn-xs down" @click="scrollToSecondLastElement" v-if="!isDown">
            <Icon class="icon" name="mynaui:arrow-down" />
        </button>
    </div>
</template>
...
const secondLast = ref(null);
const isDown = ref(false);

let observer;

const scrollToSecondLastElement = () => {
    const secondLastElement = secondLast.value;
    if (secondLastElement) {
        secondLastElement.scrollIntoView({ behavior: "smooth", block: "start" });
    }
};
...
const startWatching = () => {
    watchs = watch(
        () => chat.chat,
        (newChat) => {
            if (newChat.length > 0 && newChat[newChat.length - 2].role === "user") {
                scrollToSecondLastElement();
            }
        },
        { deep: true }
    );
...
onMounted(() => {
    ...
    if (typeof window !== "undefined") {
        ...
        if (secondLast.value) {
            observer.observe(secondLast.value);
        }
    }

    window.addEventListener("scroll", stopWatching());
});
onBeforeUnmount(() => {
    window.removeEventListener("scroll", stopWatching());
});
</script>
